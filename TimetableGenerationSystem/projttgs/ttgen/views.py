...
# Update the Data class to use your new models
class Data:
    def __init__(self):
        self._departments = Department.objects.all()
        self._sections = Section.objects.all()
        self._rooms = Room.objects.all()
        self._labs = Lab.objects.all()
        self._timings = Timing.objects.all()
        self._subjects = Subject.objects.all()
        self._practicals = Practical.objects.all()
...
# Remove duplicate methods and old model references
...
# Update the meeting_list_view function to use Timing instead of MeetingTime
@login_required
def meeting_list_view(request):
    context = {
        'meeting_times': Timing.objects.all()
    }
    return render(request, 'mtlist.html', context)

@login_required
def delete_meeting_time(request, pk):
    mt = Timing.objects.filter(pk=pk)  # Changed from MeetingTime to Timing
    if request.method == 'POST':
        mt.delete()
